...
function addToCart(productId) {
    ...
    if (existingItem) {
        ...
        showNotification(`🔥 Added another ${product.name} to demo cart!`, 'success');
    } else {
        cart.push({
            id: productId,
            name: product.name,
        ...
        showNotification(`✨ ${product.name} added to demo cart! (No real purchase)`, 'success');
    }
...
// ===== DEMO FUNCTIONALITY AND DISCLAIMERS =====

// Demo banner management
function closeDemoBanner() {
    const banner = document.querySelector('.demo-security-banner');
    if (banner) {
        ...
        // Store preference
        localStorage.setItem('kritiya-demo-banner-closed', 'true');
    }
}

function initDemoBanner() {
    const banner = document.getElementById('demoBanner');
    if (banner && !localStorage.getItem('kritiya-demo-banner-closed')) {
...
// Demo form submit handlers
function handleDemoFormSubmit(event) {
    event.preventDefault();
    
    showNotification('📋 Demo Contact Form: Message not sent - this is for demonstration only. For real inquiries, use WhatsApp or email directly.', 'info', 5000);
...
function handleDemoNewsletterSubmit(event) {
    event.preventDefault();
    
    const email = event.target.querySelector('input[type="email"]').value;
    showNotification(`📧 Demo Newsletter: No subscription created with ${email} - this is demonstration only. No emails will be sent.`, 'info', 5000);
...
function handleDemoCheckout() {
    showNotification('🛒 Demo Checkout: This is a portfolio demonstration - no payment processing or real transactions occur. Your data is safe.', 'warning', 6000);
    
    // Show demo checkout modal instead of real checkout
    setTimeout(() => {
        showDemoCheckoutModal();
    }, 1000);
}

function showDemoCheckoutModal() {
    const modalHTML = `
        <div class="modal fade" id="demoCheckoutModal" tabindex="-1" aria-hidden="true">
            <div class="modal-dialog modal-dialog-centered">
                <div class="modal-content">
                    <div class="modal-header bg-warning text-dark">
                        <h5 class="modal-title">
                            <i class="fas fa-shield-alt me-2"></i>Portfolio Demo - No Real Transactions
                        </h5>
                        <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
                    </div>
                    <div class="modal-body text-center">
                        <div class="security-notice mb-4 p-3 bg-light rounded">
                            <h5><i class="fas fa-info-circle me-2 text-primary"></i>Important Security Notice</h5>
                            <p class="mb-2"><strong>This is a portfolio demonstration website.</strong></p>
                            <p class="mb-0">No real payments, transactions, or sensitive data collection occurs. All functionality is simulated for showcase purposes only.</p>
                        </div>
                        
                        <div class="demo-checkout-steps">
                            <div class="step">
                                <i class="fas fa-shopping-cart fa-2x mb-2 text-primary"></i>
                                <h6>Demo Cart Review</h6>
                                <p class="small">In a real application, your selected items would be reviewed here with proper security</p>
                            </div>
                            <hr>
                            <div class="step">
                                <i class="fas fa-shield-alt fa-2x mb-2 text-success"></i>
                                <h6>Secure Payment Gateway</h6>
                                <p class="small">Real applications would use certified payment processors (Stripe, PayPal, Razorpay) with SSL encryption</p>
                            </div>
                            <hr>
                            <div class="step">
                                <i class="fas fa-check-circle fa-2x mb-2 text-success"></i>
                                <h6>Order Confirmation</h6>
                                <p class="small">Customers would receive secure email confirmations and order tracking</p>
                            </div>
                        </div>
                        
                        <div class="alert alert-info mt-4">
                            <i class="fas fa-lightbulb me-2"></i>
                            <strong>For real inquiries:</strong> Please contact us via WhatsApp or email for actual orders and customization requests. This ensures secure communication outside of this demo environment.
                        </div>

                        <div class="alert alert-warning mt-3">
                            <i class="fas fa-exclamation-triangle me-2"></i>
                            <strong>No Data Collection:</strong> This website does not store, process, or transmit any personal or payment information.
                        </div>
                    </div>
                    <div class="modal-footer">
                        <button type="button" class="btn btn-secondary" data-bs-dismiss="modal">
                            <i class="fas fa-times me-2"></i>Close Demo
                        </button>
                        <a href="https://wa.me/919142619991?text=Hi%20Kritiya!%20I'm%20interested%20in%20your%20custom%20clothing%20and%20would%20like%20to%20discuss%20a%20real%20order." 
                           class="btn btn-success" target="_blank" rel="noopener noreferrer">
                            <i class="fab fa-whatsapp me-2"></i>Contact for Real Order (Secure)
                        </a>
                    </div>
                </div>
            </div>
    ...
    const existingModal = document.getElementById('demoCheckoutModal');
    if (existingModal) {
...
function showDemoInfoModal() {
    const modalHTML = `
        <div class="modal fade demo-info-modal" id="demoInfoModal" tabindex="-1" aria-hidden="true">
            <div class="modal-dialog modal-lg modal-dialog-centered">
                <div class="modal-content">
                    <div class="modal-header">
                        <h5 class="modal-title">
                            <i class="fas fa-shield-alt me-2"></i>Portfolio Demo Security Information
                        </h5>
                    ...
                    <div class="modal-body">
                        <div class="row">
                            <div class="col-md-6">
                                <h6><i class="fas fa-info-circle me-2 text-primary"></i>What This Website Is:</h6>
                                <ul class="security-features">
                                    <li><i class="fas fa-check"></i>Portfolio demonstration project</li>
                                    <li><i class="fas fa-check"></i>Web development showcase</li>
                                    <li><i class="fas fa-check"></i>Educational and showcase purposes</li>
                                    <li><i class="fas fa-check"></i>No real business operations</li>
                                </ul>
                            </div>
                            <div class="col-md-6">
                                <h6><i class="fas fa-shield-alt me-2 text-success"></i>Security Measures:</h6>
                                <ul class="security-features">
                                    <li><i class="fas fa-check"></i>No data collection or storage</li>
                                    <li><i class="fas fa-check"></i>No payment processing</li>
                                    <li><i class="fas fa-check"></i>No user account creation</li>
                                    <li><i class="fas fa-check"></i>All forms are demonstration only</li>
                                </ul>
                            </div>
                        </div>
                        ...
                        <div class="alert alert-info">
                            <h6><i class="fas fa-lightbulb me-2"></i>Why This Notice Exists:</h6>
                            <p class="mb-2">This banner and these security notices help browsers and users understand that:</p>
                            <ul class="mb-0">
                                <li>This is a portfolio/demonstration website</li>
                                <li>No sensitive data is being collected</li>
                                <li>All e-commerce functionality is simulated</li>
                                <li>The site is safe to browse and explore</li>
                            </ul>
                        </div>
                        
                        <div class="alert alert-warning">
                            <h6><i class="fas fa-exclamation-triangle me-2"></i>For Real Business Inquiries:</h6>
                            <p class="mb-0">If you're interested in actual custom clothing or web development services, please contact us through the secure WhatsApp link or email provided on the website.</p>
                        </div>
                    </div>
                    <div class="modal-footer">
    ...
    // Remove existing modal if any
    const existingModal = document.getElementById('demoInfoModal');
    if (existingModal) {
        existingModal.remove();
    }
...
function showDemoAlert(feature) {
    showNotification(`🔐 ${feature} Demo: This would connect to a secure authentication system in a real application. No actual login/registration occurs on this demo site.`, 'info', 5000);
    return false;
}
